...
class BookService {
    ...
    private func createRequest(query: String, page: Int) throws -> URLRequest {
    ...
    // 외부에서 호출할 함수
    func search(query: String, page: Int = 1, completion: @escaping (Result<BookResponse, Error>) -> Void) {
        do {
            let request = try createRequest(query: query, page: page)
